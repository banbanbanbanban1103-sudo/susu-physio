...
function renderCalendar() {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
        ...
        // အဲဒီရက်မှာ ရှိသမျှ ရက်ချိန်းအားလုံးကို စစ်ထုတ်မယ်
        ...
        // ရက်ချိန်းရှိလျှင် အစက်ပြပြီး အရေအတွက်ပါ သိနိုင်အောင် (ဥပမာ- ၅ ယောက်ဆိုရင် ပိုသိသာစေရန်)
        if (hasAppt) {
            dayEl.classList.add('has-appt');
            const dot = document.createElement('span');
            dot.style.fontSize = '10px';
            dot.style.display = 'block';
            dot.innerText = '•'; // အစက်လေးပြမယ်
...
// ၃။ ရက်စွဲတစ်ခုကို နှိပ်လျှင် လူနာအားလုံး (၄-၅ ယောက်စလုံး) ကို စာရင်းလိုက်ပြရန်
function showAppointments(dateStr) {
    ...
    // Filter သုံးပြီး အဲဒီနေ့အတွက် လူနာအားလုံးကို ယူခြင်း
    const dayAppts = appointments.filter(a => a.date === dateStr);
    ...
    if (dayAppts.length > 0) {
        // အချိန်အစောဆုံးလူနာကို ထိပ်ကပြရန် စီခြင်း
    ...
    // Pop-up ပေါ်လာရင် မြင်သာအောင် အပေါ်ကို နည်းနည်းတင်ပေးခြင်း
    window.scrollTo({ top: dayApptsSection.offsetTop - 20, behavior: 'smooth' });
}
...
function rebookPatient(name, phone, address) {
    ...
    showToast(`${name} အတွက် ရက်စွဲအသစ် ရွေးချယ်ပေးပါ`);
}
